from __future__ import division

class Flower(object):
    def __init__(self, x, y, pollen):
        self.x = x
        self.y = y
        self._color = [173, 225, 251, 100]
        self.radius = 35
        self.collision_threshold = self.radius*2
        self.pollen = pollen
        self.max_pollen = pollen
        self.max_per_bee = 15000
        self.regenerate_rate = 0.1
        self.regen = 0
        self.spawn_rate = 0.01
        self.number_of_leaves = 8
        
    def render(self):
        pushMatrix()
        translate(self.x, self.y)
        self._color[-1] = constrain((self.pollen*255)/10000, 50, 255)
        fill(*self._color)
        stroke(255)
        ellipseMode(CENTER)
        for i in range(self.number_of_leaves):
            rotate(PI*i*2/self.number_of_leaves)
            ellipse(0, 0, self.radius*2, self.radius/2.0)
                
        r, g, b, a = self._color
        step_r = (0 - r) / (self.radius - 1)
        step_g = (0 - g) / (self.radius - 1)
        step_b = (0 - b) / (self.radius - 1)
        
        # Stroke circle
        fill(r,g,b,a)
        circle(0, 0, self.radius)
        noStroke()        
        for i in range(self.radius-1, 1, -1):
            r += step_r
            g += step_g
            b += step_b
            fill(r,g,b,a)
            circle(0, 0, i)
        rotate(PI)
        fill(0)

        textAlign(CENTER, CENTER)
        text_with_border(str(self.pollen), (0,0,0), (255,255,255), 0, 0)
        popMatrix()
    
    def update(self):
        self.regen += self.regenerate_rate
        if self.regen >= 1:
            self.regen = 0
            self.pollen += 1
            
def text_with_border(string, stroke_color, fill_color, x, y):
    fill(*stroke_color)
    text(string, x-2, y)
    text(string, x+2, y)
    text(string, x, y-2) 
    text(string, x, y+2) 
    fill(*fill_color)
    text(string, x, y) 
