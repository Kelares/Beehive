...
class Bee(object):
    def __init__(self, x, y, angle, hive):
        ...
        self.hive = hive
        self.observed = None
        self.duration_waggle_dance = 200
        self.counter_waggle_dance = 0
        self.spawn_flower_check = False
        self.life_span = randint(500,1000)
        self.lifetime = 0
        ...
        if self.activity == Activity.HARVESTING:
            if abs(int(self.x - self.flower.x)) == 0 and abs(int(self.y - self.flower.y)) == 0:
                if self.spawn_flower_check:
                    self.spawn_flower_check = False
                    if uniform(0,1) <= self.flower.spawn_rate:
                        return Flower(
                            x = constrain(self.flower.x + random(-width/10, width/10), 0, width),
                            y = constrain(self.flower.y + random(-height/10, height/10), 0, height),
